...
- **Score da fonte (grounding).** Rode `score.py` passando o texto do documento: `python3 <repo>/engine/score.py dados/claims-source.json dados/verdicts-source.json dados/scores-source.json --doc <texto_da_lei>`. Claim-fonte nasce LEVE (0) e só pesa se o `excerpt` for localizável no documento (grounding rastreável, não verdade). Excerpt inventado = score 0 = "não confie".
...
8. **Checar pré-condição (gate parametrizável — código, não você).** Rode `engine.descent.descent_gate(claims, verdicts, policy, min_fraction, accepted)` (ver README §14). NÃO julgue você mesmo se pode descer — a decisão é determinística. A `policy` vem do projeto/humano: `strict` (todos resolvidos), `vinculante_only`, `threshold`, `provisional` (sempre desce, marca filhos), `force`. Se `allowed` é False, **pare** e liste `blocked_by`. Chão absoluto: `CONTRADIZ-NIVEL-ACIMA` aberto bloqueia em QUALQUER política. O score (passo 5) é o termômetro; o gate é por **status**. Descer sobre fundação aberta não é proibido — é uma escolha de política, e o que ela custa fica marcado (passo 9).

9. **Expandir.** Invoque o subagent `expander`, passando os claims resolvidos do nível atual. Ele gera `dados/claims-nivel-N.json` — os claims do novo nível, cada um com `derivation` (`[Implicado]`/`[Compatível]`/`[Especulativo]`), `derives_from` e `parent_constraint`. O filho herda o score do pai via `derives_from` — o cálculo de score (passo 5) cuida disso. **Depois de gerar, rode `engine.descent.refresh_provisional(filhos, decisao["provisional_parents"])`**: todo filho que deriva de pai ainda aberto ganha a marca `provisional` (cicatriz), nasce flutuando e reabre quando o pai mudar (propagação fiada na descida). Marca `provisional` (pai não-assentado) é eixo distinto de `[Especulativo]` (derivação inventada).
